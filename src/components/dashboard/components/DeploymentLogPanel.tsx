...
function getLeetCodeHeatColor(count: number, isToday: boolean) {
  if (count === 0) return `bg-black/5 dark:bg-white/[0.03] border-border/50 dark:border-white/[0.05] ${isToday ? 'ring-1 ring-primary/40' : ''}`;
...
export function DeploymentLogPanel() {
  ...
  return (
    <motion.div variants={itemVariants} className="col-span-12 rounded-[24px] border border-border/50 dark:border-white/[0.04] bg-card px-6 py-4 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button onClick={() => startTransition(() => setSelectedYear((y) => y - 1))} className="w-7 h-7 flex items-center justify-center rounded-full border border-border/50 dark:border-white/[0.06] text-muted-foreground hover:text-foreground hover:bg-black/5 dark:bg-white/[0.02] transition-colors">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <div className="rounded-full border border-border/50 dark:border-white/[0.06] bg-black/5 dark:bg-white/[0.02] px-3 py-1 text-xs font-semibold text-foreground">
            {selectedYear}
          </div>
          <button onClick={() => canGoNext && startTransition(() => setSelectedYear((y) => y + 1))} disabled={!canGoNext} className="w-7 h-7 flex items-center justify-center rounded-full border border-border/50 dark:border-white/[0.06] text-muted-foreground hover:text-foreground hover:bg-black/5 dark:bg-white/[0.02] transition-colors disabled:opacity-30">
