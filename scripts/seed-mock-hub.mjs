...
function getShuffledOptions(correctStr, distractors, seed) {
...
function generateCompanyQuestions(company, assignIdx, diff) {
  const list = [];
...
const ASSESSMENTS = ASSESSMENTS_RAW.map(a => ({
  ...
  company_tags: a.companyTags,
  is_active: true,
}));

// Compile all questions for database seeding
const QUESTIONS = [];
for (const a of ASSESSMENTS_RAW) {
  const isTcs = a.id.startsWith('tcs-');
  const compTag = isTcs ? 'TCS' : (
    a.id.startsWith('amz-') ? 'Amazon' : (
      a.id.startsWith('acc-') ? 'Accenture' : (
        a.id.startsWith('cog-') ? 'Cognizant' : (
          a.id.startsWith('inf-') ? 'Infosys' : (
            a.id.startsWith('wip-') ? 'Wipro' : (
              a.id.startsWith('cap-') ? 'Capgemini' : 'Deloitte'
            )
          )
        )
      )
  ...
  const clientQuestions = generateCompanyQuestions(compTag, assignIdx, a.difficulty);
  
  // Map client camelCase properties to database snake_case columns
  const dbQuestionsForAssessment = clientQuestions.map(q => ({
    id: q.id,
    assessment_id: a.id,
    title: q.title,
    content: q.content,
    ...
    solution_explanation: q.solutionExplanation,
    estimated_time_seconds: q.estimatedTimeSeconds,
  }));

  QUESTIONS.push(...dbQuestionsForAssessment);
}
...
async function seed() {
  ...
  // 2. Upsert questions (in batches of 50 for safety)
  ...
  for (let i = 0; i < QUESTIONS.length; i += batchSize) {
    ...
    console.log(`   Batch ${Math.floor(i/batchSize)+1}/${Math.ceil(QUESTIONS.length/batchSize)}: ${qRes.status} ${qRes.ok ? '✅' : '❌'} (${batch.length} questions)`);
